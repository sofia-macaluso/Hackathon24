...
class MyGUI:
    ...
    def __init__(self):
        ...
        #lABEL
        self.label = Label(self.root, padx=1200, pady=20, text="2024 Election Survey", fg="white", bg="red", font=('Fredoka', 26))
        self.label.pack()
        #First Prompt: have you voted before?
        self.label = Label(self.root, padx=1200, pady=30, text="Have you ever voted before?", font=('Fredoka', 18))
        self.label.pack()
        b1 = self.button = Button(self.root, padx=60, pady=17, text="Yes!", bg="blue", fg="white", font=('Fredoka', 20), command=self.first_prompt_yes)
        b1.pack()
        b2=self.button = Button(self.root, padx=45, pady=15, text="Not yet", bg="blue", fg="white", font=('Fredoka', 20), command=self.first_prompt_no)
        b2.pack()
        self.root.mainloop()
    ...
    vote_info = {
        ...
        'missesota':'https://www.sos.state.mn.us/elections-voting/register-to-vote/#:~:text=Download%20a%20voter%20registration%20form%20to',
    ...
    def first_prompt_no(self):
        b2_label = Label(self.root, padx=1200, pady=40, text="How old are you?", bg="red", fg="white", font=('Fredoka', 18))
        b2_label.pack()
        ...
        def click():
            ...
            if age < 18:
                ...
                # call state function dictionary thing
                self.vote_func()
        b2_button2 = Button(self.root, text="enter", font=('Fredoka', 16), command=click)
        b2_button2.pack()

    def vote_func(self):
        #both cases hit enter...
        z_label = Label(self.root, padx=1200, pady=40, text="What state are you currently living in?", bg="red", fg="white", font=('Fredoka', 18))
        z_label.pack()
        state = Entry(self.root)
        state.pack()
        def click_two():
            stute = state.get()
            q_label = Label(self.root,text="Nice state! Here's " + str(stute) + "'s voter registration website:\n" + str(self.vote_info[stute]) + "\nYou are over 18, which means you are old enough to vote! Check the provided website to check other eligibilities and how to register!",font=('Fredoka', 12))
            if stute in self.vote_info:
                q_label.pack()
        b4_button = Button(self.root, text="enter", font=('Fredoka', 16), command=click_two)
        b4_button.pack()
